function [ output_args ] = K_tot_VRPM_rect( s,g,t,d,dm,N,res_B )

%   KB_VRPM: the function estimates the flux factor K_B to calculate the
...
%     R_h=R_coreback-delta_h;
    ...
    vec_KB(h)=4*alpha/pi*(gamma_1-gamma_3/3+gamma_5/5);
    vec_KB(h);
    
    vec_KE(h)=alpha*gamma_1;
    vec_KE(h);
...
K_B=mean(vec_KB);
K_E=mean(vec_KE);
gamma_out=mean(vec_gamma);

output_args=[K_B K_E gamma_out]
